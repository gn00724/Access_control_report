...
#DataZone
WorkerDicList = {"jasonc":"周世杰","timmy":"周宇澤","balah":"張瑞東","les":"李秉倫","gregt":"蔡佳儒","Jessik":"郭小嬋","wesliec":"章逸翔","john12926":"黃冠璋","scotth":"黃思嘉","nick":"黃韋翔"}
startDate = ""
EndDate = ""
workerdb_class = ""
workerdb_dic = {}
counttingDay = []
normalWorkingHour = datetime.strptime("09:00","%H:%M")
...
try:
	os.system("svn log http://192.168.1.83/svn/k3c_game_dev/Trunk/Client/K3C/Assets -r " + "{" +startDate + "}" + ":" + "{" + EndDate + "}" + " > ./_process/SVNLog_History.txt")
...
for x in keys:
	...
	for y in m_date:
		try:
			...
			if weekday < 6 and diclocate.name in c :
...
#			d_str = ""
		...
		try:
			if workerHour.hour <= 8:
				if workerHour.minute >= 30 and workerHour.hour == 8:
					None
				else:
